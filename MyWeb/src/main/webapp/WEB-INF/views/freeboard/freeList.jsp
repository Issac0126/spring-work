...
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %>
    ...
    <script>
        ...
        window.onload = function(){ //윈도우가 로딩 되면 function을 하라.

            //사용자가 페이지 관련 버튼을 클릭했을 때,
            //기존에는 각각의 a태그의 href에 각각 다른 url을 일일이 전부 작성해서 보내줬다면
            //이번에는 클릭한 버튼이 무엇인지를 확인해서 해당 버튼에 맞는 페이지 정보를
            //자바스크립트로 끌고와서 요청을 보내줄 것이다.

            document.getElementById('pagination').addEventListener('click', e => {
                if(!e.target.matches('a')) return; //이벤트 발생이 a태그가 아니면 허용X
                
                e.preventDefault();// a태그의 고유 기능을 중지함.
                //현재 이벤트가 발생한 요소(버튼)의
                ...
                //그 안에 숨겨져 있는 pageNum이라는 input 태그의 value에 
                //위에서 얻은 data-pagenum의 값을 삽입한 후 submit을 진행
