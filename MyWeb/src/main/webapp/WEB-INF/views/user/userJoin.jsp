        ...
        <div class="row">
            <div class="col-lg-6 col-md-9 col-sm-12 join-form">
                ...
                <form action="${pageContext.request.contextPath}/user/join" method="post" name="joinForm">
                    ...
                    <div class="form-group">
                        <input type="text" class="form-control" name="userBasic" id="addrBasic" placeholder="기본주소">
                    </div>
                    <div class="form-group">
                        <input type="text" class="form-control" name="userDetail" id="addrDetail" placeholder="상세주소">
                    </div>
...
<script src="//t1.daumcdn.net/mapjsapi/bundle/postcode/prod/postcode.v2.js"></script>
    ...
    <script>
        ...
        document.getElementById('idCheckBtn').onclick = function() {
            ...
            if(!idFlag){
                alert('양식에 맞는 아이디로 중복 체크를 진행해주세요.')
                return;
            }
            ...
            */
            ...
            /*
                ...
                할 수 있게 해준다. (Promise: 비동기 통신의 순서를 보장하는 문법)
                - fetch 함수가 리턴하는 Promise 객체는 단순한 응답 JSON 데이터가 아닌
                전체적이고, 포괄적인 응답 정보를 가지고 있다.
                - 따라서, 서버가 응답한 여러 정보 중 JSON 데이터만 소비하려면
                json()이라는 메서드를 사용한다.
                - 문자열 데이터라면 text() 메서드를 사용한다.
            */
            ...
            //Promise 객체의 상태가 요청 성공일 시 데이터 후속 처리 진행. (then은 Promise의 함수)
            .then(res => {
                //fetch 함수를 통해 비동기 통신이 실행되고, 
                //요청이 완료 된 후 then()의 매개값으로 응답에 관련된
                //함수를 콜백 방식으로 전달한다.
                //함수의 매개변수를 선언하면 해당 매개변수로 응답에 관련된
                //전반적인 정보를 가진 응답 정보가 전달된다.
                // console.log(res);
                // console.log(res.text);
                return res.text()
            }) //리턴된 Promise 함수에게 다시 than.
            .then(data =>{
                console.log(data);
            ...
            */
            ...
            fetch('${pageContext.request.contextPath}/user/idCheck', reqObj)
                .then(res => res.text()) //요청 완료 후 응답 정보에서 텍스트만 빼기
                    .then(data => { //텍스트만 뺀 promise 객체로부터 data 전달  
                        if(data === 'ok'){
                            //더 이상 아이디를 작성할 수 없도록 막아준다.
            ...
            fetch('${pageContext.request.contextPath}/user/mailCheck?email='+email)
                .then(res => res.text())
                    .then(data => {
                        ...
                        code = data;
                        //비활성된 인증번호 입력창을 활성화
                        document.querySelector('.mail-check-box > input[type="text"]').disabled = false;
        ...
        document.querySelector('.mail-check-input').onblur = function(e){
            ...
            if(inputCode === code ){
                ...
                // document.querySelector('.mail-check-box > input[type="text"]').disabled = true;
        ...
        //다음 주소 api 사용해 보기 (script src 추가 해야한다.)
    ...
    document.getElementById('joinBtn').onclick = function() {
        ...
        if(idFlag && pwFlag){
            if(!document.getElementById('userId').getAttribute('readonly')){ //readonly가 걸려있는지 (중복체크 여부)
        ...
        id.onkeyup = function() {
            ...
            */
            ...
            // 해석: [A~Z, a~z, 0~9]만 허용한다.(=한글 비허용) {4자 이상, 12자 이하}여야 한다. 
            ...
            if(regex.test(document.getElementById("userId").value )) {
                ...
                document.getElementById("msgId").innerHTML = "아이디는 4자 이상, 12자 이하만 가능합니다.";
                idFlag = false;
            }
        }
        /*비밀번호 형식 검사 스크립트*/
